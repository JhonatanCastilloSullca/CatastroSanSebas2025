...
class FichaIndividualEdit extends Component
{
    ...
    public function mount(Ficha $fichaanterior)
    {
        ...
        if($fichaanterior->litigantes!="")
        {
            ...
            foreach($fichaanterior->litigantes as $i => $litigante){
                $this->tipolitigante[$i]=$litigante->persona->tipo_doc;
                $this->numedoc[$i]=$litigante->persona->nume_doc;
                $this->codi_contribuye[$i]=$litigante->codi_contribuye;
                $this->nombres[$i]=$litigante->persona->nombres;
                $this->ape_materno[$i]=$litigante->persona->ape_paterno;
                $this->ape_paterno[$i]=$litigante->persona->ape_materno;
            }
        }
            ...
            if($this->condtitular!="05")
            {
                if($this->tipoTitular==1)
                {
                    ...
                    if($this->numedoc3!=""){
                        ...
                        $idBuscar=str_pad($nume_ficha,11,'0',STR_PAD_LEFT).'1200';
                        $buscarpersona3=Persona::where('id_persona',$idBuscar)->first();
                        if($buscarpersona3)
                        {
                            $persona= $buscarpersona3;
                        }else{
